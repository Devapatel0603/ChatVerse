...
cookie_dict = {
    ...
    "__Secure-1PSIDTS": "sidts-CjEBPVxjSryphUuISRiXCE_ocgMcExqcx5FdeFMVxhu1EkugIvlPBKUnLnTECneH5Dg_EAA",
    "__Secure-1PSIDCC": "ACA-OxPlvyPyknYBPg4_lgpGuj_iNl29jCMn1y4JiXZLlfRT1xbphzCbVFz0ToL3zqDxguQoi3Y"
}
...
def email_verify():
    if request.method == 'POST':
        ...
        if str(user_otp) == str(otp):
            ...
            flash('Invalid OTP, please enter valid email.')
...
def contact():
    ...
    if form.validate_on_submit() and request.method == 'POST':
        ...
        entry = Message(name=name, email=email, message=message)
        db.session.add(entry)
        db.session.commit()
